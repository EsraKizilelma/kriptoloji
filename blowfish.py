...
def key(input_key):
    ...
    #Diffie-Hellman Key Exchange algorithm kullanmak istedim fakat gerçekleştiremedim
    input_key = str(input_key)

    # Rastgele anahtar oluştur
    random_key = [random.randint(0, 0xFF) for _ in range(len(input_key))]

    # Her bir karakteri ASCII değeri ile XOR yap
    result_key = [ord(a) ^ (b & 0xFF) for a, b in zip(input_key, random_key)]

    return  result_key
  
'''
    #burada dışarıdan key alıp onunla XOR yapmam gerek
    key = [random.randint(0, 0xFFFFFFFF) for _ in range(14)]
    print(type(key))
    key = [ord(a) ^ (b & 0xFFFFFFFF) for a, b in zip(input_key, key)]     #iki listenin elemanlarını birleştirir ve ardından bu elemanlar üzerinde XOR işlemi yapılır
    print(type(key))
    return key
'''
...
print("Bellek Kullanımı",tracemalloc.get_traced_memory())
tracemalloc.stop()
